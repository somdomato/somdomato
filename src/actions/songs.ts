...
export async function lastSongs() {
  // Últimas 10 músicas
  const latest = await db.select({ id: songs.id, title: songs.title, artist: songs.artist, cover: songs.cover, playedAt: history.createdAt })
    .from(history)
...
export async function topSongs() {
  ...
  const allHistory = await db
    .select({ id: songs.id, title: songs.title, artist: songs.artist, cover: songs.cover })
    .from(history)
    .innerJoin(songs, eq(history.songId, songs.id));
  ...
  const top = Array.from(map.values()).sort((a, b) => b.count - a.count).slice(0, 10);
