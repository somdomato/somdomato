...
export default async function AdminPanel({
  ...
  const { data: historyList, total: historyTotal } =
    await getHistory(historyPage);
  const { data: requestsList, total: requestsTotal } =
    await getRequests(requestsPage);
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <li key={song.id} className="border-b py-2">
                <div className="flex justify-between items-start gap-2">
                  <div>
                    ...
                    <div className="text-xs text-gray-500">{song.path}</div>
                  </div>
                  <div>
        ...
        <section>
          ...
          <div className="flex gap-2">
            ...
            <span className="px-2 py-1">
              Página {requestsPage} de {requestsTotalPages}
            </span>
            <a
              href={`?requestsPage=${requestsPage + 1}${page ? `&page=${page}` : ""}${historyPage ? `&historyPage=${historyPage}` : ""}`}
