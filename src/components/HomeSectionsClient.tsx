...
export default function HomeSectionsClient({
  ...
  const [upcoming, setUpcoming] =
    React.useState<UpcomingEntry[]>(initialUpcoming);
  const [nextIfNoRequests] = React.useState<UpcomingEntry | null>(
    initialNextIfNoRequests,
  );
          ...
          nextIfNoRequests ? (
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-muted truncate">
                  {nextIfNoRequests.artist} • selecionada pelo AutoDJ
                </div>
              </div>
            </div>
          ) : (
