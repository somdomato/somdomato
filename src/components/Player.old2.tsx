...
export default function IcecastPlayer({ streamUrl = "https://radio.somdomato.com/geral.mp3", coverImage = "/images/logotipo.svg", defaultTitle = "Ao Vivo", defaultArtist = "Rádio", className = "" }: IcecastPlayerProps) {
  ...
  return (
    <div className={`flex items-center gap-3 bg-gradient-to-r from-slate-900 to-slate-800 rounded-lg px-3 py-1.5 shadow-lg border-2 border-black/50 ${className}`}>
      ...
      <div className="flex-1 min-w-0 hidden sm:block">
        <div className="text-sm font-semibold text-white truncate">{metadata.title}</div>
      ...
      <div className="flex items-center gap-1.5 sm:gap-2">
        ...
        <div className="hidden md:flex items-center gap-2 ml-1">
          <button onClick={toggleMute} className="w-8 h-8 flex items-center justify-center rounded-full bg-slate-700/50 hover:bg-slate-600/50 text-slate-300 hover:text-white transition-all active:scale-95" aria-label={isMuted ? "Unmute" : "Mute"}>
            {isMuted ? <VolumeX className="w-4 h-4" /> : <Volume2 className="w-4 h-4" />}
          </button>

          {/* Volume Slider */}
          <div className="relative w-20 h-1 bg-slate-700 rounded-full overflow-hidden group">
            <div className="absolute h-full bg-gradient-to-r from-blue-500 to-blue-600 transition-all" style={{ width: `${isMuted ? 0 : volume}%` }} />
