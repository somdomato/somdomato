      ...
      <div className="bg-background-alt rounded-lg shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col border-2 border-primary/30">
        ...
        <div className="flex justify-between items-center p-4 sm:p-6 border-b border-primary/30 bg-gradient-to-r from-background-alt to-background">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl sm:text-2xl font-bold text-primary">Pedir Música</h2>
          </div>
          <button onClick={onClose} className="text-gray-400 hover:text-white transition-colors p-2 hover:bg-background rounded-full">
            <X size={24} />
          </button>
        </div>
        ...
        <div className="p-4 sm:p-6 border-b border-primary/30 bg-background/50">
          ...
          <p className="text-xs text-gray-400 mt-2">{total > 0 ? `${total} música${total !== 1 ? "s" : ""} encontrada${total !== 1 ? "s" : ""}` : "Digite para buscar músicas"}</p>
        </div>
                  ...
                  {songs.map((song) => (
                    <tr key={song.id} className="border-b border-primary/10 hover:bg-background/50 transition-colors">
                      <td className="px-3 sm:px-4 py-3">
                        <img src={song.cover || "/images/logotipo.svg"} alt={song.title} className="w-10 h-10 sm:w-12 sm:h-12 object-cover rounded shadow-md" />
                      </td>
                      <td className="px-3 sm:px-4 py-3">
